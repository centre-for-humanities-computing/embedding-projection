...
train_hiddenstates = MultiLingMPNET.embed_with_meanpooled_hidden_states(loader.train_texts, cache_path="../data/processed/embeddings/hiddenstates/training_fiction4.npz", return_numpy=True)
test_hiddenstates = MultiLingMPNET.embed_with_meanpooled_hidden_states(loader.test_texts, cache_path="../data/processed/embeddings/hiddenstates/testing_fiction4.npz", return_numpy=True)
...
# %%
# Loop over all layers and run projection/correlation for each
results = []
num_layers = len(train_hiddenstates)
for layer_idx in range(num_layers):
    # Get meanpooled embeddings for this layer
    train_layer = pd.DataFrame(train_hiddenstates[layer_idx])  # shape: (n_train, hidden_dim)
    train_layer["label"] = loader.train_labels
    test_layer = pd.DataFrame(test_hiddenstates[layer_idx])    # shape: (n_test, hidden_dim)
    test_layer["label"] = loader.test_labels

    # Project test onto train subspace
    analyzer = ProjectionAnalyzer(matrix_concept=train_layer, matrix_project=test_layer)
    analyzer.project()
    x_raw = analyzer.projected_in_1D
    # Ensure x_raw is a numpy array for reshape
    if isinstance(x_raw, pd.Series):
        x_raw = x_raw.values

    # Get gold labels (as in your code)
    y = loader.test_df['label'].to_numpy()
    # Standardize labels if needed (as in your main code)
    y = (y - y.mean()) / y.std() if y.max() > 5 else (y - 3) / 2

    # Rescale projections to [-1, 1]
    scaler = MinMaxScaler((-1, 1))
    scaled = scaler.fit_transform(x_raw.reshape(-1, 1)).flatten()
    ...
    print(f"Layer {layer_idx}: Spearman ρ = {spearman_corr:.2f} (p={spearman_p:.2f}), Pearson r = {pearson_corr:.2f} (p={pearson_p:.2f})")

# Save results to CSV
results_df = pd.DataFrame(results)
results_df.to_csv("../data/processed/embeddings/hiddenstates/layerwise_projection_results.csv", index=False)
print("Saved layerwise projection correlation results.")
# %%
